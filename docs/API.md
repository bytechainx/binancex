...
| 模块 | 职责与示例入口 |
| --- | --- |
| `value::spot` / `parse::spot` | Spot 响应；`parse_spot_exchange_info`、`parse_spot_trade`、`parse_spot_kline`、`parse_spot_ticker_price`、`parse_spot_book_snapshot` |
...
| `value::coinm` / `parse::coinm` | COINM 响应；`parse_coinm_exchange_info`、`parse_coinm_kline`、`parse_coinm_open_interest`、`parse_coinm_book_snapshot` |
| `value::options` / `parse::options` | Options 响应；`parse_options_exchange_info`、`parse_options_mark`、`parse_options_kline`、`parse_options_book_snapshot` |
...
wire 字段使用源 JSON 键，Rust 字段使用 snake_case。当前对象字段按冻结树的可缺性保留为 `Option<T>`；缺项不是零值。wire string 通常保留为 `String`；`SpotReferencePrice.reference_price` 显式使用 `Decimal`。wire number 采用 `serde_json::Number` 保留表示。各类原始时间字段保持源单位，不自动跨族转换。解析器的结构与语义限制见 [标准](标准.md)。
...
`Decimal::new(&str)` 校验十进制文本并保留原文，接受可选正负号、小数点及科学计数指数；有效数字与出现的指数都必须含数字，拒绝空串、孤立符号和重复小数点。`as_str()` 返回原始表示，`sign()` 返回数值符号。`Quantity::new(Decimal, QuantityUnit)` 从已校验数值派生只读符号；单位为 `BaseAsset`、`QuoteAsset` 或 `Contracts`。`-0` 的文本得以保留，符号为 `Sign::Zero`。调用方须先依据端点合同确定单位。
...
通用 `parse_exchange_info` 仅接受 `spot`、`usdm`、`coinm`、`options`，并调用对应 exchangeInfo 严格解析器后计算原始字节摘要；它不验证端点版本、标的资格或授权。`WhitelistSnapshot` 字段可公开构造，因此持有快照本身也不是通过验证的凭证。
